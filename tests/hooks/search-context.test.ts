...
describe("search-context hook", () => {
  ...
  it("returns context for standard prompt", () => {
    const output = processSearchHook(db, "how do React hooks work?", memRepo, sessRepo, config);
  ...
  it("returns empty for very short prompt", () => {
    const output = processSearchHook(db, "yes", memRepo, sessRepo, config);
    expect(output).toBe("");
  });
  ...
  it("debits tokens from session budget", () => {
    ...
    processSearchHook(db, "how do React hooks work?", memRepo, sessRepo, config);
